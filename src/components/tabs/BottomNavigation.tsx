...
export default function BottomNavigation(): React.ReactElement {
  ...
  const scrollToItem = useCallback((index: number): void => {
    if (scrollContainerRef.current) {
      const itemElement = scrollContainerRef.current.children[0]?.children[index] as HTMLElement;
      if (itemElement) {
  ...
  useEffect(() => {
    const activeIndex = filteredNavigationItems.findIndex(item => pathname.startsWith(item.path));
    if (activeIndex !== -1) {
  ...
  return (
    <nav 
      className="fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-gray-200 z-50 dark:bg-gray-800 dark:border-gray-700"
    >
      <div 
