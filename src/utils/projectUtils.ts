 ...
 * 取得風險概率權重
 */
function getProbabilityWeight(probability: 'low' | 'medium' | 'high'): number {
  switch (probability) {
    case 'low': return 1;
 ...
 * 取得風險影響權重
 */
function getImpactWeight(impact: 'low' | 'medium' | 'high'): number {
  switch (impact) {
    case 'low': return 1;
    case 'medium': return 3;
    case 'high': return 5;
...
export function calculateCostPerformanceIndex(project: Project): number {
  if (!project.financialMetrics) return 1;
  
  const { earnedValue, budgetVariance } = project.financialMetrics;
  const actualCost = earnedValue - budgetVariance;
 ...
 * 計算預算差異百分比
 */
export function calculateBudgetVariancePercentage(project: Project): number {
  if (!project.financialMetrics) return 0;
  
  const { budgetVariance, earnedValue } = project.financialMetrics;
  if (earnedValue === 0) return 0;
  
  return Math.round((budgetVariance / earnedValue) * 100);
}
...
function calculateFinancialHealthScore(project: Project): number {
  if (!project.financialMetrics) return 100;

  const cpi = calculateCostPerformanceIndex(project);
  const budgetVariance = calculateBudgetVariancePercentage(project);

  let score = 100;

  // CPI 影響
  if (cpi < 0.9) score -= 30;
  else if (cpi < 1.0) score -= 15;

  // 預算差異影響
  if (Math.abs(budgetVariance) > 20) score -= 25;
  else if (Math.abs(budgetVariance) > 10) score -= 15;
 ...
 * 取得即將到期的里程碑
 */
export function getUpcomingMilestones(milestones: ProjectMilestone[], daysThreshold: number = 7): ProjectMilestone[] {
  const now = new Date();
  const thresholdDate = new Date(now.getTime() + daysThreshold * 24 * 60 * 60 * 1000);

  return milestones.filter(milestone => {
    if (milestone.status === 'completed') return false;
    const targetDate = milestone.targetDate && typeof milestone.targetDate === 'object' && 'toDate' in milestone.targetDate
      ? (milestone.targetDate as { toDate: () => Date }).toDate()
