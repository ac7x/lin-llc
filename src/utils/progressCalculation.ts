...
export function calculateWorkpackageProgress(
    ...
    if (!workpackage.subWorkpackages || workpackage.subWorkpackages.length === 0) {
        ...
        const relevantLogs = activityLogs.filter(log =>
            log.workpackageId === workpackage.id || log.workpackageId.startsWith(`${workpackage.id}:`)
        );

        // 如果有相關記錄，根據活動記錄計算進度
        if (relevantLogs.length > 0) {
            const avgProgress = relevantLogs.reduce((sum, log) => sum + log.progress, 0) / relevantLogs.length;
...
export function updateWorkpackagesProgress(
    ...
    return workpackages.map(wp => {
        ...
        // 更新狀態基於進度
        if (updatedWp.progress === 0) {
            updatedWp.status = "新建立";
        } else if (updatedWp.progress < 100) {
            updatedWp.status = "進行中";
        } else {
            updatedWp.status = "已完成";
        }
