    ...
    <div className="container mx-auto p-4 max-w-4xl">
      ...
      {developerNote && (
        <div className="mb-8 p-4 border-l-4 border-indigo-500 bg-indigo-50 dark:bg-gray-800 rounded-r-lg shadow">
          <h2 className="text-xl font-semibold text-indigo-800 dark:text-indigo-300">開發者日誌 (版本: {developerNote.version})</h2>
      ...
      {user && (
        <form onSubmit={handleSubmit} className="mb-8 p-4 bg-gray-100 dark:bg-gray-800 rounded-lg shadow-md">
            {isDeveloper && (
                 <input
                    type="text"
                    value={version}
            ...
            <textarea
                ...
                placeholder={isDeveloper ? "輸入開發者日誌..." : (canPost ? "請輸入您的留言..." : "您今天已經留過言了，請明天再來！")}
                rows={4}
      ...
      <div className="space-y-4">
        <h2 className="text-2xl font-bold mt-8 mb-4 border-b pb-2 text-gray-900 dark:text-white">所有留言</h2>
        {messages.filter(msg => !msg.isDeveloperNote).map(msg => (
          <div key={msg.id} className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-md flex items-start space-x-4">
            {msg.userPhotoURL ? 
              <Image src={msg.userPhotoURL} alt={msg.userName || 'user avatar'} width={40} height={40} className="w-10 h-10 rounded-full"/>
              :
            ...
            <div className="flex-1">
              <div className="flex justify-between items-center">
                <span className="font-semibold text-gray-800 dark:text-gray-200">{msg.userName}</span>
                <span className="text-xs text-gray-500 dark:text-gray-400">
                  {msg.createdAt?.toDate().toLocaleString()}
                </span>
              </div>
              <p className="mt-1 text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{msg.message}</p>
            </div>
          </div>
        ))}
