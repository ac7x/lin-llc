...
export function NotificationBell({
  ...
  return (
    <Link href='/notifications' className={`relative inline-block ${className}`}>
      {unreadCount > 0 ? (
        <BellSolidIcon
          className={`${sizeClasses[size]} text-blue-500 hover:text-blue-600 transition-colors`}
        />
      ) : (
        <BellIcon
          className={`${sizeClasses[size]} text-gray-500 hover:text-gray-600 dark:text-gray-400 dark:hover:text-gray-300 transition-colors`}
        />
      )}
...
export function NotificationSummary({ className = '' }: NotificationSummaryProps) {
  ...
  return (
    ...
    >
      <BellIcon className='h-4 w-4' />
      <span>{unreadCount > 0 ? `${unreadCount} 則未讀通知` : '沒有未讀通知'}</span>
    </Link>
  );
}
